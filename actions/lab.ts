...
export const bookLabTest = async (values: z.infer<typeof LabTestSchema>) => {
  try {
    ...
    const testEndTime = new Date(testStartTime.getTime() + 60 * 60 * 1000); // Assuming 1-hour test duration
    ...
    // Create the new lab test booking
    ...
    return { success: "Lab test booked successfully.", test: createdLabTest };
